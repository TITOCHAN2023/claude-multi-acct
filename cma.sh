...
# 启动 claude 时"永远附带"的额外参数 (默认空; 逃生阀, 一般不用)。
# --dangerously-skip-permissions / --remote-control 不在这里, 改为每账号可开关(默认关)。
: "${CMA_CLAUDE_FLAGS:=}"

_cma_reserved="add rm ls list next link unlink set default help -h --help"

# 每账号两个开关(标记文件存在=开, 默认关):
#   .cma-flag-skip -> --dangerously-skip-permissions
#   .cma-flag-rc   -> --remote-control
...
_cma_run_default() {
  ...
  command claude $CMA_CLAUDE_FLAGS $(_cma_flags_for "$CMA_HOME") "$@"
}
...
# flag 存储目录: default -> CMA_HOME (垫底账号), 其余 -> 该账号目录
_cma_flagdir() {
  if [ "$1" = default ]; then printf '%s' "$CMA_HOME"; else _cma_dir "$1"; fi
}

# 按开关状态拼出启动 flag (默认全关 -> 空)
_cma_flags_for() {
  local fd="$1" out=""
  [ -f "$fd/.cma-flag-skip" ] && out="$out --dangerously-skip-permissions"
  [ -f "$fd/.cma-flag-rc" ]   && out="$out --remote-control"
  printf '%s' "$out"
}
...
_cma_flags_label() {
  local fd="$1" l=""
  [ -f "$fd/.cma-flag-skip" ] && l="skip"
  [ -f "$fd/.cma-flag-rc" ]   && l="${l:+$l+}rc"
  printf '%s' "${l:--}"
}

# 开关设置: cc2 set <账号|default> <skip|rc> <on|off>
_cma_set() {
  local name="$1" key="$2" val="$3" fd f
  if [ -z "$name" ] || [ -z "$key" ] || [ -z "$val" ]; then
    echo "用法: cc2 set <账号|default> <skip|rc> <on|off>" >&2; return 1
  fi
  fd="$(_cma_flagdir "$name")"
  if [ "$name" != default ] && [ ! -d "$fd" ]; then echo "cc2: 账号 '$name' 不存在" >&2; return 1; fi
  case "$key" in
    skip|perm|permissions) f=".cma-flag-skip" ;;
    rc|remote|remote-control) f=".cma-flag-rc" ;;
    *) echo "cc2: 未知开关 '$key' (可用: skip / rc)" >&2; return 1 ;;
  esac
  case "$val" in
    on|1|true|yes)  touch "$fd/$f" && echo "$name: $key = on" ;;
    off|0|false|no) rm -f "$fd/$f" && echo "$name: $key = off" ;;
    *) echo "cc2: 值只能是 on / off" >&2; return 1 ;;
  esac
}
...
_cma_launch() {
  ...
  env CLAUDE_CONFIG_DIR="$dir" command claude $CMA_CLAUDE_FLAGS $(_cma_flags_for "$dir") "$@"
}
...
_cma_ls() {
  mkdir -p "$CMA_HOME"
  local any=0 d base name status mode fmt="  %-12s %-6s %-9s %-30s %s\n"
  echo "账号根目录: $CMA_HOME   (启动参数默认全关, 用 cc2 set 开关)"
  printf "$fmt" "账号" "模式" "启动参数" "登录邮箱" "凭证"
  echo "  -----------------------------------------------------------------------------"
  printf "$fmt" "默认(垫底)" "-" "$(_cma_flags_label "$CMA_HOME")" "$(_cma_email "$HOME")" "cc 垫底,永不修改"
  for d in "$CMA_HOME"/*/; do
    ...
    printf "$fmt" "$name" "$mode" "$(_cma_flags_label "$base")" "$(_cma_email "$base")" "$status"
  done
  [ "$any" = 0 ] && echo "  (还没有账号, 用 cc2 add <名字> 添加)"
  echo "  启动参数: skip=--dangerously-skip-permissions  rc=--remote-control"
  return 0
}
...
cc2 — 官网多账号并行 / 轮询 (默认账号永远垫底, 不会被本工具修改)
  ...
  cc2 ls                    列出账号 / 模式 / 启动参数开关 / 登录邮箱 / 凭证状态
...
说明:
  ...
  * --dangerously-skip-permissions / --remote-control 是每账号可开关的状态, 默认关闭,
    cc2 set 打开; default 指垫底账号。cc2 ls 可查看每个账号当前开关。
